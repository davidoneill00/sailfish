...
class BinaryInspiral(SetupBase):
    ...
    def do_inspiral(self, time):
        if (time <= self.inspiral_start_time * self.reference_time_scale):
            ...
            raise ValueError("Inspiral flag failed. The current time is t = %g, the inspiral start time is tstart = %g nd the inspiral time takes tgw = %g"%(time/2/3.14159, self.inspiral_start_time), self.inspiral_time_list[-1]/2/3.14159)
...
class CoolInspiral(SetupBase):
    ...
    @property
    def _base_cooling_coefficient(self):
    ...
    def physics(self):
        if self.is_isothermal:
            ...
            return dict(
                ...
                buffer_driving_rate=1000.0,  # default value in circumbinary.py
                buffer_onset_width=0.1,  # default value in circumbinary.py
                base_coefficient=self._base_cooling_coefficient,
                constant_softening=self.constant_softening,
                viscosity_model=ViscosityModel.CONSTANT_ALPHA if self.alpha > 0.0 else ViscosityModel.NONE,
                viscosity_coefficient=0.0,
                alpha=self.alpha,
